...
vec2 randomGradient(vec2 p) {
  ...
  gradient = sin(gradient);
  gradient = sin(gradient + u_time / 2.) + 3.716 + cos(gradient + u_time / 1.280) + 3.260;
  return gradient;
}
...
float domainWarpFbmPerlinNoise(vec2 uv) {
    ...
    const float octaves = 9.632;
    
    for (float i = 0.0; i < octaves; i++) {
        fbm1 = fbmNoise(uv + 0.4 * fbm1 + vec2(0.0 ,0.));
	    fbm2 = fbmNoise(uv + 0.4 * fbm2 + vec2(0., 0.));
        }
...
float dwNoise = domainWarpFbmPerlinNoise(uv);
  ...
  color = vec3(1. - pow(abs(dwNoise), 0.5 )) ;
